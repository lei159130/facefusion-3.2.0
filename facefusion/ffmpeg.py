...
def extract_frames(target_path: str, temp_video_resolution: str, temp_video_fps: Fps, trim_frame_start: int,
	...
	with tqdm(total=extract_frame_total, desc=wording.get('extracting'), unit='frame', ascii=' =',
...
def read_audio_buffer(target_path: str, audio_sample_rate: int, audio_sample_size: int, audio_channel_total: int) -> \
...
def merge_video(target_path: str, temp_video_fps: Fps, output_video_resolution: str, output_video_fps: Fps,
				trim_frame_start: int, trim_frame_end: int) -> bool:
	merge_frame_total = predict_video_frame_total(target_path, output_video_fps, trim_frame_start, trim_frame_end)
	temp_file_path = get_temp_file_path(target_path)
	temp_frames_pattern = get_temp_frames_pattern(target_path, '%08d')

	is_gif = get_file_format(target_path).lower() == 'gif'

	if is_gif:
		palette_path = temp_file_path.replace('.gif', '_palette.png')

		palette_commands = ffmpeg_builder.chain(
			ffmpeg_builder.set_conditional_fps(temp_video_fps),
			ffmpeg_builder.set_input(temp_frames_pattern),
			['-vf',
			 f'scale=16x16,trim=start_frame={trim_frame_start}:end_frame={trim_frame_end},fps={output_video_fps},palettegen=stats_mode=diff'],
			ffmpeg_builder.force_output(palette_path)
		)

		with tqdm(total=merge_frame_total, desc=wording.get('generating_palette'), unit='frame', ascii=' =',
				  disable=state_manager.get_item('log_level') in ['warn', 'error']) as progress:
			palette_process = run_ffmpeg_with_progress(palette_commands,
													   lambda frame_number: progress.update(frame_number - progress.n))
			if palette_process.returncode != 0:
				logger.error(f"调色板生成失败: {palette_process.stderr}", __name__)
				return False

		gif_commands = ffmpeg_builder.chain(
			ffmpeg_builder.set_conditional_fps(temp_video_fps),
			ffmpeg_builder.set_input(temp_frames_pattern),
			ffmpeg_builder.set_input(palette_path),
			['-filter_complex',
			 f'[0:v]trim=start_frame={trim_frame_start}:end_frame={trim_frame_end},fps={output_video_fps},scale={output_video_resolution}[x];[x][1]paletteuse'],
			['-f', 'gif'],
			ffmpeg_builder.force_output(temp_file_path)
		)

		with tqdm(total=merge_frame_total, desc=wording.get('generating_gif'), unit='frame', ascii=' =',
				  disable=state_manager.get_item('log_level') in ['warn', 'error']) as progress:
			gif_process = run_ffmpeg_with_progress(gif_commands,
												   lambda frame_number: progress.update(frame_number - progress.n))

			if gif_process.returncode != 0:
				logger.error(f"GIF生成失败: {gif_process.stderr}", __name__)
				return False
			...
			try:
				...
				logger.warn(f"删除调色板文件失败: {e}", __name__)
		...
		with tqdm(total=merge_frame_total, desc=wording.get('merging'), unit='frame', ascii=' =',
				  disable=state_manager.get_item('log_level') in ['warn', 'error']) as progress:
			process = run_ffmpeg_with_progress(commands,
											   lambda frame_number: progress.update(frame_number - progress.n))
			return process.returncode == 0
