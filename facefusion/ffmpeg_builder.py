...
from facefusion.filesystem import get_file_format, find_ffmpeg_path
...
def run(commands : Commands) -> Commands:
	return [ find_ffmpeg_path(), '-loglevel', 'error' ] + commands
...
def add_filter_complex(filter_name, options=None):
	if options:
		return ['-filter_complex', f"{filter_name}={options}"]
	return ['-filter_complex', filter_name]
